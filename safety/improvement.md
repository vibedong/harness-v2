...
| --- | --- | --- |
| MCP adapter around `status`, `verify`, `preflight`, `gate`, `decision`, and `init/apply` | observation | implemented as a local stdio adapter; future work may add Codex app configuration guidance or hook-equivalent integration under a separate scope |
...
| record-density mode engine | observation | implemented as task-mode / record-strength evaluation; future work may tune source-volume and evidence-binding thresholds with measured project data |
