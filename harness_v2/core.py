...
def start_task(
    ...
    *,
    ...
    spec_path = root_path / "records" / "stages" / "spec.md"
    ...
    previous_spec_text = spec_path.read_text(encoding="utf-8") if spec_path.exists() else None
    ...
    spec_path.write_text(_registered_spec_md(task_id, normalized_title, summary_text, source_items), encoding="utf-8")
    ...
    if not validation.ok:
        ...
        if previous_spec_text is None:
            if spec_path.exists():
                spec_path.unlink()
        else:
            spec_path.write_text(previous_spec_text, encoding="utf-8")
        return TaskStartResult(
            ok=False,
            root=str(root_path),
            task=INITIAL_TASK_PATH,
    ...
    return TaskStartResult(
        ...
        task=INITIAL_TASK_PATH,
        task_id=task_id,
        written=("CURRENT.md", INITIAL_TASK_PATH, "records\\current-task.md", "records\\stages\\spec.md"),
    )
...
def _registered_spec_md(task_id: str, title: str, summary: str, source_basis: list[str]) -> str:
    source_lines = "\n".join(f"- `{item}`" for item in source_basis)
    return f"""# Spec 단계 기록
...
- 실제 작업 요청은 대화 답변만으로 흘려보내지 않습니다.
- 이 파일은 현재 작업의 첫 번째 task-local output surface입니다.
- 구현, 크롤링 실행, dependency 설치, release, publish, secret 접근, destructive action은 이 spec 기록만으로 승인되지 않습니다.
- 질문은 한 번에 하나씩 묻고, 코드/문서로 답할 수 있으면 먼저 확인합니다.
- 대화로 결정된 도메인 용어와 결정은 project-local docs 또는 records에 lazily 기록합니다.

## 다음 단계

현재 요청을 기준으로 source 조사, 질문, PRD/spec 정리를 진행합니다. 이후 구현 계획이나 파일 변경은 별도 approval, permission, proof, lifecycle 경계를 가진 amended task contract가 필요합니다.
"""
...
실제 작업 요청을 받았는데 `records\\current-task.md`와 `records\\stages\\spec.md`가 그 요청을 가리키지 않으면 하네스가 작업을 잡은 상태가 아닙니다. 이 상태에서 대화 답변만으로 기획, 분석, 구현 방향을 진행하지 않습니다.
...
4. `records\\current-task.md`와 `records\\stages\\spec.md`가 현재 사용자 요청을 가리키는지 확인합니다. 둘 중 하나라도 맞지 않으면 대화 답변만으로 진행하지 않습니다.
5. `harness-v2 verify <task.json>`로 task contract를 검증합니다.
6. 파일 변경이나 side-effectful command 전에 `harness-v2 gate <task.json> --root .`를 실행합니다.
7. local integration과 release boundary 상태를 확인할 때는 `harness-v2 doctor --root .`를 실행합니다.
8. `approval.approved_paths`에 명시된 경로만 수정합니다.
9. `approval.excluded_side_effects` 또는 `permission.denied_side_effects`에 명시된 side effect는 실행하지 않습니다.
10. 완료 전에는 `proof.obligations`의 모든 항목을 실행하거나 blocked 상태를 보고합니다.
