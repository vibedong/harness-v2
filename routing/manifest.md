...
| MCP stdio adapter | `python -m harness_v2 mcp` or `node bin\harness-v2.js mcp` with JSON-RPC over stdio | local stdio only; tools wrap existing status, verify, preflight, gate, decision, init, and apply behavior and do not replace source, approval, permission, proof, lifecycle, or release boundaries |
