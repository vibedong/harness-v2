...
현재 구현 상태: HARNESS V2는 명시적 CLI/MCP/task-contract 사용을 위한 local `workflow_binding_engine`으로 분류합니다. current gate, workflow stage rule, lifecycle transition authority, stale/backtrack anchor, approval/permission/proof decision record, preflight, gate, record density를 실행 경로에서 평가합니다. 그래도 자동 shell/editor blocker, Codex app hook installer, completion layer는 아닙니다.
...
- log만으로 lifecycle이 이동하지 못하게 하는 transition-ledger 평가
- stale source, approval, permission, proof, artifact, transition evidence 재사용을 막는 freshness/stale-backtrack 검사
- 실행 가능한 `effective_record_strength` profile을 통한 task mode / record strength 검사
- 정확한 approval, permission, proof binding을 위한 ApprovalDecision, PermissionDecision, ProofReceipt evaluator
...
HARNESS V2는 local stdio MCP adapter를 포함합니다. 이 adapter는 `status`, `verify`, `preflight`, `gate`, `decision`, `init`, `apply`를 MCP tool로 노출합니다. MCP adapter는 기존 HARNESS V2 core를 감싸는 얇은 wrapper이며, `CURRENT.md`, task contract, approval, permission, proof, lifecycle, release boundary를 대체하지 않습니다.
...
이 분류는 HARNESS V2의 명시적 local surface에 한정됩니다. 즉 task contract, CLI command, Python CLI로 위임되는 npm wrapper, local stdio MCP tool, generated scaffold, tests를 기준으로 한 분류입니다. HARNESS V2가 모든 shell, editor, network, package, Codex app 행동을 자동으로 가로막는다는 뜻은 아닙니다.

분류 근거는 다음 실행 가능한 검사입니다.

- 공식 `workflow_stage` 값과 파생 `current_gate`
- log-only 이동이 아닌 lifecycle transition evaluator
- source hash와 backtrack target이 있는 stale/backtrack detection
- task-mode와 record-density 평가
- approval, permission, proof receipt evaluator
- 제안된 side effect와 write path에 대한 preflight/gate 검사
- fresh-project scaffold verification과 existing-project preservation test
...
MCP adapter는 stdio 위에서 newline-delimited JSON-RPC를 사용합니다. 사람이 대화형 shell 명령처럼 쓰는 것이 아니라, MCP-capable client가 실행하는 용도입니다. 노출되는 tool은 `harness_status`, `harness_verify`, `harness_preflight`, `harness_gate`, `harness_decision`, `harness_init`, `harness_apply`입니다.
...
`doctor`는 통합된 local surface, 권장 `status` / `verify` / `gate` 순서, 현재 닫힌 release boundary를 보고합니다. 이것은 read-only integration check이지 release readiness가 아닙니다.
