...
- npm package `harness-v2` already exists, and `0.1.4` is the next project application version for the same package.
- GitHub CLI is authenticated for `vibedong/harness-v2`.
- npm CLI now authenticates successfully with `npm.cmd whoami`.
- The `0.1.4` transaction adds project scaffold application through `harness-v2 init --root .` and `harness-v2 apply --root .`.
- npm registry reports `harness-v2@0.1.4` with tarball `https://registry.npmjs.org/harness-v2/-/harness-v2-0.1.4.tgz`.
- Git tag `v0.1.4` was pushed to `vibedong/harness-v2`.
- GitHub release `v0.1.4` was created at `https://github.com/vibedong/harness-v2/releases/tag/v0.1.4`.
...
- Exact release execution commands are permitted only for `harness-v2@0.1.4`, `v0.1.4`, and the GitHub repository `vibedong/harness-v2`.

## Current Permission Ceiling

The current package, GitHub publish, npm wrapper, and npm release scope allows local editable package smoke verification, local Node wrapper proof, npm dry-run pack proof, exact npm publish for `harness-v2@0.1.4`, exact Git tag `v0.1.4`, exact GitHub release `v0.1.4`, and GitHub repository push, but denies:

- dependency changes;
- Python package registry publish or deploy work;
- release execution outside `harness-v2@0.1.4` / `v0.1.4`;
